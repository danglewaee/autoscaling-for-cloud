...
Calibration-aware autoscaling is a useful framework for testing whether uncertainty quality, rather than point accuracy alone, changes the cost-reliability trade-off of short-horizon scaling decisions under bursty serverless workloads.

Our contributions are threefold. First, we formulate autoscaling as a calibration-sensitive decision problem and make explicit how calibration error translates into downstream SLA error. Second, we instantiate this idea with a lightweight residual-calibrated policy that can be reproduced on a public serverless trace without a large forecasting stack. Third, we provide an empirical analysis that separates aggregate behavior from burst-only behavior, which reveals a harder failure regime than average metrics alone would suggest.
