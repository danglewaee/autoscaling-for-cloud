...
One exploratory result already points in a plausible direction. A hybrid variant that combines app-specific calibration with a one-minute reactive guard reduces burst-only SLA violation from $0.7885$ to $0.6432$ while also lowering aggregate mean cost from $1.0383$ to $0.8625$. However, it raises mean oscillation from $0.0678$ to $0.1680$, which is too reactive to present as the main method. The takeaway is useful nonetheless: the bottleneck is increasingly in the decision layer, not in the point forecaster itself.
