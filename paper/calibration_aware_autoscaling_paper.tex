...
\end{itemize}
The default implementation pools residuals globally, while one ablation calibrates residuals per application to better align with the app-conditional decision setting emphasized in the theory section. This gives a transparent calibration-aware method before introducing heavier probabilistic models.
...
The main method uses the calibrated decision rule directly. An exploratory hybrid variant augments it with a short reactive guard based on recently observed demand. Proposition~\ref{prop:burst-guard} explains the logic of that variant: it is not intended as a separate forecasting model, but as a decision-layer envelope for partially revealed bursts.
...
\begin{enumerate}[leftmargin=1.2cm]
    ...
    \item Refine the hybrid burst-aware guard so that it preserves the current burst-SLA gains without drifting toward reactive-level oscillation.
    \item Keep tail-risk penalties and app-specific calibration as decision-layer ablations before adding a heavier forecasting model.
\end{enumerate}
